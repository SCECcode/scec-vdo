...
public class ScriptingPluginGUI extends JPanel implements ActionListener, MouseListener{
	...
	public void playScene(double startTime, boolean render) {
		//create splines
		...
		double endTime = Double.parseDouble(this.endTime.getText());
		Preconditions.checkState(endTime >= startTime);
		int numFrames = Integer.parseInt(noOfFrames.getText());
		double frameRate = (double)numFrames/endTime; // start time always zero here for FPS calculation
		...
		vtkAnimationCue cue = new vtkAnimationCue();

		CueAnimator2 animator = new CueAnimator2(scene);
		animator.addRenderStepListener(
				new CameraAnimator(pointsToMoveCameraOnPosition, pointsToMoveCameraOnFocalPoint, pointsToMoveCameraOnViewUp));
		...
		if (render) {
			...
			if (!movieFile.getName().toLowerCase().endsWith(".mov"))
				movieFile = new File(movieFile.getAbsolutePath()+".mov");
			MediaLocator m = null;
			try {
				m = new MediaLocator(movieFile.toURI().toURL());
			} catch (MalformedURLException e) {
				Throwables.propagate(e);
			}
			animator.setRendering(m, frameRate);
		}

		cue.AddObserver("StartAnimationCueEvent", animator, "startCue");
		cue.AddObserver("EndAnimationCueEvent", animator, "endCue");
		cue.AddObserver("AnimationCueTickEvent", animator, "tick");
