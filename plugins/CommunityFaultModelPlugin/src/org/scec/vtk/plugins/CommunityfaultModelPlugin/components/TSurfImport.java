...
public class TSurfImport implements DataImport {
    ...
    private void readSourceFile(File file, Fault3D obj) {
     ...
     // Create a mapper
  	  ...
  	  obj.lat.SetName("latitude");
  	obj.lon.SetName("longitude");
  	vtkMutableDirectedGraph graph = new vtkMutableDirectedGraph();
            ...
            while ((line = inStream.readLine()) != null){
                ...
                if (data.hasMoreTokens()){
                    ...
                    if ( temp.equals("VRTX") || temp.equals("PVRTX")) {
                        ...
                        //for 3d depth
                        latlon[0] = Transform.calcRadius(latlon1[0]) + zVal;
                        ...
                        x = Transform.customTransform(latlon);
                        ...
                        vertexArray.InsertNextPoint(vertex);
                        continue;
                    }
        ...
        obj.colors = c;
        //obj.setFaultBranch(actorassign);
        
        //TODO remove as not using graph for cfm
        graph.GetVertexData().AddArray(obj.lat);
        graph.GetVertexData().AddArray(obj.lon);
        vtkDoubleArray zvals = new vtkDoubleArray();
        for(int i =0;i<obj.vertices.GetNumberOfPoints();i++)
	    {zvals.InsertNextValue(obj.vertices.GetPoint(i)[2]);}
        graph.GetVertexData().AddArray(zvals);
        
        vtkGraphToPolyData graphToPolyData = new vtkGraphToPolyData();
		graphToPolyData.SetInputData(graph);
		graphToPolyData.Update();
			  
        obj.graphToPlyData = graphToPolyData.GetOutput();//new vtkPolyData();
	    obj.graphToPlyData.SetPoints(obj.vertices); //graphToPolyData.GetOutput();
	    obj.graphToPlyData.SetPolys(obj.triangles);
	    /*for(int i =0;i<obj.graphToPlyData.GetNumberOfPoints();i++)
	    {
	    	double[] pt = obj.graphToPlyData.GetPoint(i);
	    	pt[2] = obj.vertices.GetPoint(i)[2] ;
	    	obj.graphToPlyData.se
	    }*/
	   // obj.graphToPlyData.SetPoints(obj.vertices);
       // obj.setDisplayed(true);
        //return actorassign;//mapperassign;//
    }
